...
public enum Config implements PeConfig {
  ...
  private final static String SERVICE_ID = "pe/streampipes.processor.flink";

  Config() {
    config = SpConfig.getSpConfig(SERVICE_ID);
    config.register(ConfigKeys.HOST, "streampipes-archetype-pe-processors-flink", "Data processor host");
    config.register(ConfigKeys.PORT, 8090, "Data processor port");
    config.register(ConfigKeys.SERVICE_NAME, "streampipes archetype pe processors flink", "Data processor service name");
